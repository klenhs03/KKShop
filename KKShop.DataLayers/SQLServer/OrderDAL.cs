    ...
    public class OrderDAL : BaseDAL, IOrderDAL
    {
...

        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                list = connection.Query<Order>(sql: sql, param: parmeters, commandType: System.Data.CommandType.Text).ToList();
                connection.Close();
            }
            return list;
        }
        ...
        {
            List<OrderStatus> ListOrderStatus();  // Đảm bảo kiểu trả về và chữ ký phương thức khớp
        }
